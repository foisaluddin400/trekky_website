...
import { useAddOutdoorMutation, useAddWasherMutation, useAddWaterPumpMutation, useGetSingleOutdoorQuery, useUpdateOutdoorMutation } from "../redux/api/routesApi";
...
const UpdateOutdoor = () => {
      const {id} = useParams();
    const {data:singleUpdate} = useGetSingleOutdoorQuery({id})
    console.log(singleUpdate)
  const navigate = useNavigate();
  const [addHeater] = useUpdateOutdoorMutation();
  ...
  //   };
     useEffect(() => {
      if (singleUpdate?.outdoorRadio
) {
        const admin = singleUpdate?.outdoorRadio
      ...
      <div className=" lg:mt-11 mt-6 px-3">
        <div className=" pb-7 lg:pb-0">
          <h1 className="text-3xl font-semibold text-[#F9B038]">
            Update Outdoor Radio Information
          </h1>
        </div>
