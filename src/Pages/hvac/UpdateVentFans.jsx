...
const UpdateVentFans = () => {
    const {id} = useParams();
    const {data:singleUpdate} = useGetSingleVentFansQuery({id})
    console.log(singleUpdate)
  const navigate = useNavigate();
  const [addHeater] = useUpdateVentFansMutation();
  ...
  //   };
   useEffect(() => {
      if (singleUpdate?.ventFans
) {
        const admin = singleUpdate?.ventFans
  ...
  <Form.Item
          ...
          <Link to={"/details/AddDishWasherInfo"}>
            <button
              type="primary"
              htmlType="submit"
