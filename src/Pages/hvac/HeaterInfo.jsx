...
import { useDeleteHeaterMutation, useGetHeaterQuery } from "../redux/api/routesApi";
...
const HeaterInfo = () => {
  const {data:heaterData} = useGetHeaterQuery();
  const [deleteHeater] = useDeleteHeaterMutation()

    const handleDelete = async (id) => {
      console.log(id);
      try {
        const res = await deleteHeater(id).unwrap();
              ...
              {heaterData?.heaters?.map((item) => (
                ...
                >
                  ...
                  <div className="flex justify-end gap-2 mt-3">
                    ...
                    <Link to={`/details/Heater/update-heater/${item._id}`}>
