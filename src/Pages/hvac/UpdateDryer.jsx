...
import { useAddDriyerMutation, useAddWasherMutation, useAddWaterPumpMutation, useGetSingleDriyerQuery, useUpdateDriyerMutation } from "../redux/api/routesApi";
...
const UpdateDryer = () => {
    const {id} = useParams();
    const {data:singleUpdate} = useGetSingleDriyerQuery({id})
    console.log(singleUpdate)
  const navigate = useNavigate();
  const [addHeater] = useUpdateDriyerMutation();
  ...
  //   };
 useEffect(() => {
      if (singleUpdate?.dryer) {
        const admin = singleUpdate?.dryer
