...
import { useAddRouterMutation, useAddWasherMutation, useAddWaterPumpMutation, useGetSingleRouterQuery, useUpdateRouterMutation } from "../redux/api/routesApi";
...
const UpdateRouter = () => {
      const {id} = useParams();
    const {data:singleUpdate} = useGetSingleRouterQuery({id})
    console.log(singleUpdate)
  const navigate = useNavigate();
  const [addHeater] = useUpdateRouterMutation();
  ...
  //   };
   useEffect(() => {
      if (singleUpdate?.wifiRouter
) {
        const admin = singleUpdate?.wifiRouter
