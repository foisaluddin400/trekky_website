...
const UpdateCallingFans = () => {
    const {id} = useParams();
    const {data:singleUpdate} = useGetSingleCallingFansQuery({id})
    console.log(singleUpdate)
  const navigate = useNavigate();
  const [addHeater] = useUpdateCallingFansMutation();
 ...
 useEffect(() => {
      if (singleUpdate?.cellingFans) {
        const admin = singleUpdate?.cellingFans
      ...
      <div className=" lg:mt-11 mt-6 px-3">
        <div className=" pb-7 lg:pb-0">
          <h1 className="text-3xl font-semibold text-[#F9B038]">
           Update Celling Fans Information
          </h1>
        </div>
...
export default UpdateCallingFans;
