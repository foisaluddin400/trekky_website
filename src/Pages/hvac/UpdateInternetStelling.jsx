...
import { useAddInternetSatettlingMutation, useAddWasherMutation, useAddWaterPumpMutation, useGetSingleInternetSatettlingQuery, useUpdateInternetSatettlingMutation } from "../redux/api/routesApi";
...
const UpdateInternetStelling = () => {
      const {id} = useParams();
    const {data:singleUpdate} = useGetSingleInternetSatettlingQuery({id})
    console.log(singleUpdate)
  const navigate = useNavigate();
  const [addHeater] = useUpdateInternetSatettlingMutation();
  ...
  //   };
     ...
     useEffect(() => {
      if (singleUpdate?.internetSatellite
) {
        const admin = singleUpdate?.internetSatellite
      ...
      <div className=" lg:mt-11 mt-6 px-3">
        <div className=" pb-7 lg:pb-0">
          <h1 className="text-3xl font-semibold text-[#F9B038]">
            Update Internet/Satellite Information
          </h1>
        </div>
...
export default UpdateInternetStelling;
