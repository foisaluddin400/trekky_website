...
import { useDeleteWasherMutation, useGetWasherQuery } from "../redux/api/routesApi";
...
const WasherInfo = () => {
  const{data} = useGetWasherQuery()
  const[deleteWasher] = useDeleteWasherMutation()
   const handleDelete = async (id) => {
      console.log(id);
      try {
        const res = await deleteWasher(id).unwrap();
              ...
              {data?.washers?.map((item) => (
                ...
                >
                  ...
                  <div className="flex justify-end gap-2 mt-3">
                    ...
                    <Link to={`/details/Washer/update-washer/${item._id}`}>
